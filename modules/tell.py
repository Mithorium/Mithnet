...
def setup(self):
    ...
    if not os.path.exists(self.tell_filename):
        try:
            with open(self.tell_filename, 'w') as f:
                f.write('')
                f.close()
        except OSError:
            pass
...
def f_remind(phenny, input):
    if (
            re.match(r"zellio", input.nick.lower()) or
            re.match(r"zellio", input.user.lower()) or
            re.match(r".*\.es\.its\.nyu\.edu", input.host.lower())):
        return
    teller = input.nick

    verb, tellees, msg = input.groups()
    verb = verb.encode('utf-8')
    tellees = tellees.encode('utf-8')
    ...
    if teller.lower() in tellees:
        ...
        if len(tellees) == 1:
            response = "I'll pass that on when {} is around.".format(tellees_original)
        else:
            response = "I'll pass that on when they're around."
...
def clearreminders(phenny, input):
    if input.nick not in phenny.ident_admin:
        return phenny.reply('Requires authorization. Use .auth to identify')
